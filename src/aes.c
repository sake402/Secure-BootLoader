...
#if 0//MULTIPLY_AS_A_FUNCTION
static uint8_t Multiply(uint8_t x, uint8_t y)
{
#if 0
	uint8_t yy = 0;
	for (uint8_t i = 0; i < 5; i++, y >>= 1){
		uint8_t yyy = y & 1;
		uint8_t xx = x;
...
//
//
//    uint8_t mul[] = {0x0B, 0x0D, 0x09, 0x0E};
//    for (int8_t ii = 0; ii < 4; ii++){
//    	uint8_t dd = 0;
//    	for (uint8_t iii = 0; iii < 4; iii++){
//    		dd ^= Multiply(abcd[iii], mul[(ii+iii)%4]);
//    	}
//        (*state)[i][3-ii] = dd;
//    }
...
void AES_ECB_encrypt(const uint8_t* input, const uint8_t* key, uint8_t* output, const uint16_t length)
{
...
void AES_ECB_decrypt(const uint8_t* input, const uint8_t* key, uint8_t *output, const uint16_t length)
{
  // Copy input to output, and work in-memory on output
  memcpy(output, input, length);
  state = (state_t*)output;
...
void AES_CBC_encrypt_buffer(uint8_t* output, uint8_t* input, uint16_t length, const uint8_t* key, const uint8_t* iv)
{
  uintptr_t i;
